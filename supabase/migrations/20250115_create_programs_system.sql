...
-- Create user_programs table to track user's program participation
...
-- RLS Policies for programs (everyone can read)
CREATE POLICY "Enable read access for all users" ON public.programs
  FOR SELECT USING (true);

-- RLS Policies for user_programs (users can only see their own)
CREATE POLICY "Enable read access for users on their own programs" ON public.user_programs
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Enable insert for users on their own programs" ON public.user_programs
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Enable update for users on their own programs" ON public.user_programs
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Enable delete for users on their own programs" ON public.user_programs
  FOR DELETE USING (auth.uid() = user_id);

-- Admin policies
CREATE POLICY "Enable all access for admins on programs" ON public.programs
  FOR ALL USING (is_admin());

CREATE POLICY "Enable all access for admins on user_programs" ON public.user_programs
  FOR ALL USING (is_admin());
...
CREATE INDEX idx_user_programs_status ON public.user_programs(status);
...
-- Function to start a program (ensures only one active program)
