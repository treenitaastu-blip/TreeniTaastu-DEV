...
BEGIN
  ...
  -- Check if 'done' column exists first, then use appropriate query
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
      AND table_name = 'userprogress' 
      AND column_name = 'done'
  ) INTO _has_done_column;
  
  IF _has_done_column THEN
    -- Use 'done' filter if column exists
    ...
    FROM user_last_day;
  ELSE
    -- Calculate without 'done' filter if column doesn't exist
    WITH user_last_day AS (
      SELECT 
        up.user_id,
        MAX(pd.day + (pd.week - 1) * 5) as last_day_number
